 ...
 * @modify 2026-08-27 状态目录统一到 Harness 根目录 ~/.dsh/diff-review：
 *                    stateDirPath / legacyStateFilePath 不再取 profile 的
 *                    ctx.baseUrl 分目录，所有 profile（web/desktop/headless）
 *                    读写同一份审查记录与 UI 偏好（旧数据自 profiles/web 迁移）。
 * @modify 2026-09-01 状态根改为 $DSH_HOME/diff-review：dshHomePath 优先读
 *                    托管环境变量 DSH_HOME（桌面端 buildHarnessSpawnOptions
 *                    注入宿主进程），未设置或仅空白时回退 ~/.dsh，与
 *                    @deepseek-ai/dsh-home-paths 的 resolveDshHome 语义
 *                    一致；旧固定根 ~/.dsh/diff-review（含 ui/ 子目录）与
 *                    旧单文件启动时一次性迁移到新根（幂等，不覆盖新根已
 *                    有文件）。
 * @modify 2026-09-14 修复「轮次」恒为 0（审查列表不显示第 N 轮）：dsh-session
 *                    的 Session 只有 snapshotEvents()/ownEvents() 方法，没有
 *                    session.events 属性——旧写法取到 undefined 后被
 *                    Array.isArray 挡掉，currentTurnOf / questionOf 静默返回空，
 *                    所有 op 的 turn 都落成 0。新增 sessionOf / eventsOf 适配
 *                    （公开 API ctx.agents.get(id).session 优先、内部注册表
 *                    兜底；事件日志走 snapshotEvents()，兼容旧 events 属性），
 *                    轮次与每轮问题恢复；修复前落盘的旧记录（turn=0）按
 *                    turn/start 事件的 time 时间戳反推回填
 *                    （turnTimelineOf / opTurn），历史会话同样能显示轮次。
 * @modify 2026-09-14 修复「点击文件名打不开」：浏览器端并没有可用的打开文件
 *                    客户端 API——旧代码调的 ctx.workspaces.openPath 在桌面端
 *                    不存在，openViaWorkspace 整个函数静默 no-op，点击毫无反应。
 *                    新增宿主路由 POST /diff-review/open（{path}），由宿主用
 *                    系统默认程序打开：spawn + 参数数组，win32 走
 *                    cmd /c start ""（windowsVerbatimArguments 自行加引号——
 *                    含 & ^ 等 cmd 元字符且不带空格的路径不会被 Node 自动加
 *                    引号，会被 cmd 当命令分隔符），darwin 走 open、其它走
 *                    xdg-open；detached + unref 且监听 error，不阻塞宿主、
 *                    spawn 失败也不会抛出未捕获异常。
 * @modify 2026-09-14 文件状态标记（新增/修改/删除）：summary / turn / file 三个
 *                    载荷的文件项新增 status 字段（'added' | 'modified' |
 *                    'deleted'），供审查列表与详情头部打徽章。删除由宿主实时
 *                    按「绝对路径是否还存在」判定（existsSync）——插件只监听
 *                    write/edit，删文件一般走 shell 命令，解析命令不可靠，磁盘
 *                    才是事实来源；相对路径且记录没有 cwd 时无法定位磁盘文件，
 *                    一律不判删除以免误报。新增 = 会话内首个 op 的 before 为
 *                    null（工具返回值语义），其余为修改；升级前无快照的旧记录
 *                    同样落到修改。
 * @modify 2026-09-14 /diff-review/file 载荷补 cwd / absPath / repoPath：侧栏
 *                    diff tab 正文（原全屏窗口）的工具栏要用它们显示仓库相对
 *                    路径、复制路径与「打开文件」（按记录时的 cwd 定位，避免
 *                    依赖客户端会话 cwd）。
 */
import { readFile, unlink, writeFile } from 'node:fs/promises'
import { mkdirSync, readFileSync, writeFileSync, existsSync, renameSync, readdirSync, statSync, copyFileSync } from 'node:fs'
...
// 旧版一致。旧固定根 ~/.dsh/diff-review 与旧单一大文件在启动时自动迁移
// 到新根；旧版单一大文件再拆分为每会话文件并改名 .bak-migrated 备份。
...
// 旧版固定状态根 ~/.dsh/diff-review（2026-08-27 版，迁移到 $DSH_HOME 用）
function legacyHomeStateDirPath() {
  return join(homedir(), DSH_HOME_DIR_NAME, STATE_DIR_NAME)
}

// 旧版固定单文件路径 ~/.dsh/diff-review-state.json（迁移用）
function legacyHomeStateFilePath() {
  return join(homedir(), DSH_HOME_DIR_NAME, LEGACY_STATE_FILE)
}
...
// 移动单个条目；跨卷 rename 失败降级为复制，复制也失败则原文件原地保留
// （数据不丢，只是未迁移）
function moveEntry(src, dest) {
  ...
  if (!renamed) {
    try {
      ...
      console.error('diff-review migrate home: copy ' + basename(src) + ' failed', e2)
    }
  }
}

// 跨卷复制兜底：递归复制文件/目录（仅 rename 无法跨卷时用到）；目标已存在
// 一律不覆盖（新根是活动数据，已迁移数据同样优先）
function copyRec(src, dest) {
  if (existsSync(dest)) return
  if (statSync(src).isDirectory()) {
    mkdirSync(dest, { recursive: true })
    for (const entry of readdirSync(src)) {
      copyRec(join(src, entry), join(dest, entry))
    }
    return
  }
...
// 把旧目录条目逐个搬到新根（含 ui/ 子目录）；目标已存在的同名文件原样保留
// （新根是活动数据，绝不覆盖）；子目录递归搬内容而不 rename 目录本身
// （目标目录可能已存在，rename 目录会整体失败）
function moveDirEntries(srcDir, destDir) {
  let entries
  try {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      // 单条失败（瞬时锁等）不影响其余条目继续迁移
      console.error('diff-review migrate home: entry ' + entry + ' failed', e)
    }
  }
}

// 一次性迁移：旧固定根 ~/.dsh/diff-review（含 ui/ 子目录）与旧单文件
// ~/.dsh/diff-review-state.json 搬到当前 Harness 家目录，状态根改到
// $DSH_HOME 后旧记录不丢；幂等——旧目录搬空/消失后后续启动即空转；
// 新根已有文件绝不覆盖（新根是活动数据，旧数据只作补充）
function migrateHomeMove() {
  const oldDir = legacyHomeStateDirPath()
  const oldFile = legacyHomeStateFilePath()
  const newDir = stateDirPath()
  const newFile = legacyStateFilePath()
  // 同一根（如 DSH_HOME 未设置回退 ~/.dsh）：无需迁移
  if (isSameDir(oldDir, newDir)) {
    return
  }
  if (!existsSync(oldDir) && !existsSync(oldFile)) {
    return
  }
  // 先确保新根存在（rename/复制都需要目标父目录在）
  mkdirSync(newDir, { recursive: true })
  // 旧单文件先行搬到新根，随后 migrateLegacyState 在新根内做拆分迁移
  if (existsSync(oldFile) && !existsSync(newFile)) {
    moveEntry(oldFile, newFile)
  }
  if (existsSync(oldDir)) {
    moveDirEntries(oldDir, newDir)
  }
}
...
function apply(ctx) {
  ...
  // 的小文件，记录随重启保留；旧固定根 ~/.dsh/diff-review 与旧版单一大文件
  // 首次启动自动迁移。
  migrateHomeMove()
  migrateLegacyState()
