...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 在右侧侧栏（dsh-better-sidebar 的 editor tab）打开文件：优先后端算好的
		// absPath；dsh-better-sidebar 服务不可用时降级为系统默认方式打开。
		// cordis 里 ctx.betterSidebar 属性直读受 inject 声明约束，未声明会取不到
		// （dsh-better-sidebar 内部也统一用 ctx.get），故先走 ctx.get 兜底回属性直读。
		function openInSidebar(f, sessionId) {
			let svc = null;
			try {
				if (ctxRef && typeof ctxRef.get === "function") { svc = ctxRef.get("betterSidebar"); }
			} catch (e) { svc = null; }
			if (!svc && ctxRef) {
				try { svc = ctxRef.betterSidebar; } catch (e) { svc = null; }
			}
			const abs = (f && f.absPath) || resolveAbsPath(sessionId, f && f.path, f && f.cwd);
			const name = String((f && f.path) || "").split(/[\/\\]/).pop() || (f && f.name) || "";
			if (svc && typeof svc.openFile === "function") {
				try {
					svc.openFile({ sessionId: sessionId, cwd: (f && f.cwd) || undefined }, abs, name);
					console.info("[diff-review] openInSidebar -> betterSidebar.openFile", abs);
					return;
				} catch (e) { console.error("[diff-review] openInSidebar failed", e); }
			}
			console.warn("[diff-review] dsh-better-sidebar 不可用，降级为系统默认打开", abs);
			openViaWorkspace(abs);
		}
		...
		// 点遮罩层空白处或「关闭」退出。
		function MaximizeDialog({ path, repoPath, onClose, onRevert, onRevertAll }) {
								...
								: data.error
									...
									: (showSections
										? React.createElement("div", null,
											...
											? React.createElement(Section, { key: i, section: sec, onRevert: data.revertible === true ? (opIndex) => revertOne(opIndex) : undefined, busy: false, onLineCtx: (e, h) => openLineMenu(e, path, h, setMenu, repoPath) })
											: null))
		...
		function LivePanel(props) {
			...
			// 关闭预览后面板「固化」的标准高度：约 1/3 视口、上限 380px——
			// 文件很多时若用自适应会顶满 600px 上限，视觉上等于没收缩
			const defaultPanelH = () => Math.max(220, Math.min(Math.round(window.innerHeight * 0.32), 380));
			// 展开后手动拖拽的高度（px）；null 用默认自适应。初始 null，
			...
			// 同时镜像到 savedPanelHRef，供关闭预览时还原到用户拖拽保存的高度
			const savedPanelHRef = React.useRef(undefined);
			React.useEffect(() => {
				if (!session) return;
				let alive = true;
				fetch("/diff-review/ui-prefs?session=" + encodeURIComponent(session)).then((r) => r.json()).then((v) => {
					const h = v && v.prefs && v.prefs.panelH;
					if (alive && Number.isFinite(h) && h >= 96) {
						savedPanelHRef.current = h;
						// 仅当当前无显式高度（未被拖拽/撑大）时才应用，避免覆盖用户操作
						setPanelH((cur) => (cur == null ? h : cur));
					}
				}).catch(() => {});
				return () => { alive = false; };
			}, [session]);
			...
			// 打开文件详情后自适应撑高：detail 已解除自身 440 限高（style maxHeight:none），
			// 溢出被 list（overflow-y:auto）吸收——因此测量「list 的溢出量」
			// （scrollHeight - clientHeight），把面板撑大该差值；只增不减、够高不动，
			// 自动撑高温和上限（视口 60%、≤640px）但绝不压缩已拖拽的高度，
			// 超出上限仍由滚动兜底。
			// 用 useLayoutEffect 在浏览器绘制前同步测量并调整高度，避免
			// 「先以旧高度渲染全高内容、下一帧才撑大」造成的闪烁；
			// 注意：必须位于早退 return 之前——hook 数量不能随渲染路径变化
			React.useLayoutEffect(() => {
				if (!openPath || !detail) return;
				const card = cardRef.current;
				if (!card) return;
				const list = card.querySelector(".dsdrv-livepanel-list");
				if (!list) return;
				const over = list.scrollHeight - list.clientHeight;
				if (over > 2) {
					// 自动撑高用温和上限（视口 60%、≤640px），但钳制上限取
					// max(自动上限, 当前面板高度)——「只增不减」：够高时不动，
					// 也绝不把用户已拖拽得更高的面板压回自动上限
					const autoMax = Math.min(Math.max(200, Math.round(window.innerHeight * 0.6)), 640);
					const base = panelH != null ? panelH : card.clientHeight;
					const capH = Math.max(autoMax, base);
					setPanelH(Math.min(capH, Math.max(96, base + over)));
				}
			}, [openPath, detail]);
			if (!session || allList.length === 0) return null;
			const toggle = (p) => {
				// 高度策略：① 点其他文件（切换）：高度保持不变（已撑定就不动）；
				// ② 再点当前文件（关闭预览）：面板重新计算——回到默认自适应高度，
				// 收紧到刚好容纳文件列表；下次打开文件仍会按需只增不减地撑大
				if (openRef.current === p) {
					// 关闭预览：实时从 Host 读一次该会话保存的面板高度并还原。
					// 先用已知镜像/标准值乐观占位，响应返回后以最新存储值为准；
					// 这样即使拖拽刚保存过、内存镜像未更新，也能读到正确值
					openRef.current = null; setOpenPath(null); setDetail(null);
					const applyH = (h) => setPanelH(Number.isFinite(h) && h >= 96 ? h : defaultPanelH());
					applyH(savedPanelHRef.current);
					if (session) {
						fetch("/diff-review/ui-prefs?session=" + encodeURIComponent(session)).then((r) => r.json()).then((v) => {
							const h = v && v.prefs && v.prefs.panelH;
							savedPanelHRef.current = Number.isFinite(h) && h >= 96 ? h : undefined;
							applyH(h);
						}).catch(() => {});
					}
					return;
...
// onClick 阻断冒泡，避免拖拽结束的 click 落到标题栏误触折叠
						...
						React.createElement("div", { className: "dsdrv-livepanel-list" + (allList.length > 5 && !openPath && !panelH ? " dsdrv-livepanel-list-scroll" : "") },
						allList.map((f) => {
							...
							// 行上只显示文件名（basename）；仓库地址仅在展开后的文件地址
							// （drv-detail-path）显示，此处保持简洁，超长省略尾部、悬停看全路径
							const baseName = String(f.path || "").split(/[\/\\]/).pop() || f.name || "";
							return React.createElement("div", { key: f.path, className: "dsdrv-livepanel-file" },
								React.createElement("button", { type: "button", className: "dsdrv-livepanel-row", onClick: () => toggle(f.path) },
									React.createElement("span", {
										...
										title: f.absPath || f.path,
										style: isLatest ? { color: colors.latestName, fontWeight: 600 } : undefined,
										onClick: (e) => { e.preventDefault(); e.stopPropagation(); openInSidebar(f, session); }
									}, baseName),
								...
								open ? React.createElement("div", { className: "dsdrv-livepanel-detail", style: { maxHeight: "none" } },
									React.createElement("div", { className: "drv-detail-toolbar" },
										React.createElement("span", { className: "drv-detail-path drv-path-link", title: f.absPath || f.path, onClick: (e) => { e.preventDefault(); e.stopPropagation(); openInSidebar(f, session); } }, f.repoPath || f.path),
										React.createElement("span", { className: "drv-header-spacer" }),
										React.createElement("button", { className: "drv-btn", title: "复制仓库地址（相对仓库根，无仓库时复制完整路径）", onClick: () => copyText(f.repoPath || f.path) }, "复制路径"),
									...
									detail === null
										...
										: (detail.sections && detail.sections.length > 0
											? detail.sections.map((sec, i) => React.createElement(Section, { key: i, section: sec, onRevert: (opIndex) => revertOp(f.path, opIndex), busy: false, onLineCtx: (e, h) => openLineMenu(e, f.path, h, setMenu, f.repoPath) }))
											: React.createElement("div", { className: "drv-empty" }, "该文件没有可展示的修改"))) : null);
						}))))),
					maximized ? React.createElement(MaximizeDialog, { path: maximized, repoPath: (allList.find((x) => x.path === maximized) || {}).repoPath, onClose: () => setMaximized(null), onRevert: revertOp, onRevertAll: (p) => revertAll(p) }) : null,
					React.createElement(CtxMenu, { menu, onClose: () => setMenu(null) }),
...
.dsdrv-livepanel-name { flex:none; white-space:nowrap; font-weight:500; }
