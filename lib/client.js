...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const LIGHT = { addBg: "#e6ffec", addFg: "#1a7f37", delBg: "#ffebe9", delFg: "#cf222e", ctxBg: "#f6f8fa", gutter: "#57606a", badgeBg: "#0969da", badgeFg: "#ffffff", turnAdd: "#1a7f37", turnDel: "#cf222e", turnBg: "rgba(255, 183, 77, 0.1)", turnBorder: "#ffb74d", latestName: "#0969da" };
		const DARK = { addBg: "#10251c", addFg: "#7ee787", delBg: "#2d1415", delFg: "#ffa198", ctxBg: "#161b22", gutter: "#8b949e", badgeBg: "#4493f8", badgeFg: "#0d1117", turnAdd: "#7ee787", turnDel: "#ffa198", turnBg: "rgba(255, 183, 77, 0.1)", turnBorder: "#ffb74d", latestName: "#4493f8" };
		...
		function LivePanel(props) {
			...
			// 同轮各文件通常为同一问题，避免重复）
			const latestSummary = "最近变动 - " + (() => {
				const names = titleList.slice(0, 2).map((f) => String(f.path || "").split(/[\/\\]/).pop() || f.name || "");
				const q = titleList.map(qOf).find((x) => x) || "";
				return names.join(" | ") + (q ? "：" + q : "");
			})();
			// 当前展开的文件被清空/移除时自动收起
			React.useEffect(() => {
			...
			return React.createElement(React.Fragment, null,
				...
				React.createElement("div", { className: "dsdrv-livepanel-head" + (collapsed ? "" : " dsdrv-livepanel-head-open"), title: collapsed ? "展开审查列表" : "折叠审查列表", onClick: () => setCollapsed(!collapsed) },
					...
					titleList.length > 0 ? React.createElement("span", { className: "dsdrv-livepanel-summary", title: latestSummary }, latestSummary) : null,
					React.createElement("span", { className: "drv-header-spacer" }),
				...
				collapsed
					? null
					: React.createElement("div", { className: "dsdrv-livepanel-list" + (allList.length > 5 && !openPath ? " dsdrv-livepanel-list-scroll" : "") },
						allList.map((f) => {
							...
							return React.createElement("div", { key: f.path, className: "dsdrv-livepanel-file" },
								React.createElement("button", { type: "button", className: "dsdrv-livepanel-row", onClick: () => toggle(f.path) },
									React.createElement("span", { className: "dsdrv-livepanel-name" + (isLatest ? " dsdrv-livepanel-name-latest" : ""), style: isLatest ? { color: colors.latestName, fontWeight: 600 } : undefined }, String(f.path || "").split(/[\/\\]/).pop() || f.name || ""),
